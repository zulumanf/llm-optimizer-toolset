# AGENTS.md — How to Think in This Repository

This file tells Codex how to think. It is not documentation — it is operating instructions.
...
We measure how AI assistants (ChatGPT, Codex, Gemini, Perplexity, etc.) mention, describe, and recommend each client versus its competitors. We run controlled, versioned prompt experiments; capture raw responses immutably; score them with transparent, versioned methodology; and turn findings into evidence-backed tasks.
...
Codex should never be "figuring out" what to build. Product decisions live in `docs/`. Executable feature specs live in `specs/`. If a request has no spec, the first deliverable is a spec, not code.
