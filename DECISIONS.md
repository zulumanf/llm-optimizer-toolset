...
## 2026-09-07 — Entity verification is a hard send gate; positive replies are owned; hand-off needs no sequence

- **Every count-stating email requires `entity_resolution_verified`** (Touch 1, follow-ups, corrections, report deliveries, and rewrites whose ancestor carries the frozen evidence), for BOTH the prospect and the competitor, regardless of numerator or gap. Verified means: a team's RealTrends lead aliases are applied (or covered by the name), an individual's canonical person record exists, or an operator recorded the entity level from an authoritative page. Anything else fails closed as `ENTITY_RESOLUTION_UNVERIFIED`. Why: 17 of 183 campaign emails carried a count that changed on re-resolution; 7 inverted the claim. The old draft QA compared the email to its snapshot, never the snapshot to the entity.
- **Report hand-off (spec 129) enqueues any canonical positive reply** — Gmail-synced or hand-recorded, sequence or not — and `REPORT_HANDOFF_AUTOSEND` is now opt-in (`"true"`). Why: both real "yes" replies had failed the old preconditions and no hand-off ever ran; automation prepares, the founder sends.
- **A positive reply is owned the moment it is recorded** (owner = the user who set the legal sender identity, next action, due next business day) and stays on Today until a human records the outcome (`positive_reply_resolved` activity). Why: Steve Wall sat three days after his report with no owner or next action.
- **Refused follow-ups re-plan only for operational refusals** (mailbox cap, brokerage frequency, transport); reply/DNC/correction/entity/evidence/exclusivity refusals pause the sequence for a human. Why: the scheduler previously re-rendered every parked touch each morning.
- **`decline` is a reply class** with objection flags (`PRICE_TOO_HIGH`, `PREFERS_DIY`, …); the classifier strips quoted history on every path. Never suppresses.
- **Credible engagement only**: audit views inside 5 minutes of publish are the operator's publish-QA loads; opens come from the classified open-signal view (scanners excluded); reply rows are canonical per prospect+received_at (corrections don't double count); QA131 fixtures are archived and excluded.
- **Dead automation**: the daily control-tower trigger is disabled (14/14 failures, no notification connector). Sense-check at publish stays advisory; the delivery-time sense-check inside the hand-off is the QA promise that matters. Weekly briefs and refresh outputs are left running (no cost, no noise) until a client exists.

## 2026-09-10 — One evidence release layer; the snapshot is verified against the world, not just the copy against the snapshot (spec 136)

- **Every customer-facing competitive claim passes `verifyEvidenceRelease` at approval, at sequence resume, at hand-off QA and again at transmission.** Entity identity (both sides), licensed relationship provenance, production record/period/metric/level comparability, frozen run presence, provider isolation, completeness, denominator, both counts and correction state are re-verified over the frozen snapshot. Why: spec 130 proved a snapshot can be internally consistent and externally incomplete (Blu House: 11 stated, 29 real); validating copy against its snapshot cannot catch that.
- **Two counting paths must agree exactly.** The canonical SQL counter stays primary; a plain-JS shadow recount over the same immutable rows re-derives provider isolation, error/holdout exclusion, current revision, echo exclusion and one-credit-per-answer. Disagreement fails closed as `PRIMARY_SHADOW_COUNT_MISMATCH`/`DENOMINATOR_MISMATCH` — never averaged, never "trust primary". Why: common-mode software failure.
- **A zero is only a zero when it was searched.** Every verified identity name (company, registry aliases, RealTrends team-lead forms) is searched in the raw valid answers; a hit with no current mention row is `ALIAS_COVERAGE_UNVERIFIED` (and `ZERO_NOT_VERIFIED` when the claim states 0). The check is not classification; it forces reconciliation before release.
- **Run completeness for claims is per claimed provider: every expected cell (non-holdout prompts × configured repetitions) must be a valid capture.** Spec 124's "completed or partial with a real scope" stays the eligibility rule; this is the smallest fail-closed refinement for outreach. Cohort runs read `partial` only because Perplexity cells failed; OpenAI is 256/256 on every one. Why: a partial claimed-provider run understates absolute counts even when the denominator excludes failures.
- **Reasons are deterministic codes** (`PROSPECT_ENTITY_UNVERIFIED`, `ENTITY_LEVEL_MISMATCH`, `PENDING_CORRECTION`, …), ledgered with reconstruction diagnostics on `gate_verdict` (`prospect-send-gate-v3`). No `QA_FAILED`.
- **No new tables.** Verification is derived; history (sends, snapshots, corrections, responses, mentions) stays insert-only.

## 2026-09-11 — Autonomous fulfillment lane: one evidence source, computed facts, narrow autonomy in SHADOW (spec 137)

- **The spec 129 hand-off row is the workflow instance; no new workflow platform.** Statuses became an explicit machine (`pending → autonomy_eligible → evidence_verified → report_published → qa_passed → release_ready → scheduled → sent`, `needs_review`/`stopped`), every write is a guarded transition, and `needs_review` exits only by founder reactivation (back to the eligibility check) or a stop. Why: the existing table already had the semantics; `lib/workflow` is the graph engine for definitions, not a per-reply instance store.
- **Escalation happens at classification, before preparation.** A positive reply that is not a plain "send it" (pricing, guarantee, scope, methodology, question, hedge, long text, autoresponder) parks immediately with `ESCALATED_TO_FOUNDER: <reason>`; no report is auto-published for it. Why: precision over coverage — the founder's one-click publish still exists, and an auto-prepared artifact on an ambiguous reply is a temptation to send it.
- **One fact manifest per verified evidence state; artifacts assert against it.** `derivedClaimFigures` (spec 136) is the only arithmetic; the email's comparison sentence is compiled from manifest displays and cites fact ids; the report's headline facts and every number in the email body above the signature must be manifest numbers. Manifest compiles only from a VERIFIED verdict with known entity levels on both sides (UNKNOWN never becomes a fact). The manifest hash includes the correction id, so a correction changes the hash and every artifact goes stale.
- **Send-time revalidation is a hash comparison, not a rerun.** `fulfillmentSendRecheck` recompiles the manifest from a fresh release verdict and compares hashes inside the send gate (`fulfillment_manifest_current`); held (SHADOW) handoffs re-check on every tick.
- **Effectively-once sends by fingerprint, not a boolean.** The staged draft is the durable send intent (unique `send_intent_key`); the outgoing mail carries our own RFC `Message-ID`; a stale dispatcher claim is reconciled by `rfc822msgid:` search and ledgered as `reconciled_from` — never resent blindly. The ledger stays insert-only.
- **One adversarial reviewer, after every deterministic check.** `fulfillment_release_review` (PASS/BLOCK with quoted reasons) replaces the recipient-review + sense-check pair inside the lane; it judges wording only. Why: more reviewers did not catch the historical failures, which were upstream evidence failures.
- **Modes are founder-set, default SHADOW.** SHADOW prepares everything and records `auto_verdict = would_send` for comparison with what the founder sends; CANARY is a deterministic per-handoff bucket; the kill switch holds in every mode; promotion is never automatic. `REPORT_HANDOFF_AUTOSEND=true` maps to NARROW_AUTONOMOUS only when no mode is set.
...
- **Idempotency is content-addressed at three layers.** `generation_key` (prospect + manifest hash + every version) dedupes the artifact; narration WAVs are keyed by script hash + voice + segment so a retry never re-charges ElevenLabs; the canonical MP4 key is deterministic and `storeArtifact({ reuseExisting })` returns the existing row rather than re-encoding.
- **Staleness reuses the email's send-time recheck.** `checkBinding` calls `fulfillmentSendRecheck` (fresh verdict → manifest hash) at three points (before TTS, after render, before publish) and at release; a correction anywhere in flight makes the artifact `stale`, which is a sink — a new revision comes from the next lane pass, never a requeue.
- **Connective copy is a versioned constant, not a model output.** Only the ONE semantic reviewer touches a model, and it returns a verdict; no text from it enters the script. Why: a constrained generator still has to be reviewed, and the deterministic sentences already carry the meaning.
- **Distribution is an adapter; V1 is storage.** The canonical MP4 lives under the evidence root and is referenced as `local_storage` / ACCESS_GATED (served later through the private report's session gate). Unlisted YouTube is declared SHAREABLE_BY_LINK and refuses until credentials exist.
- **Preparation parks, never spends, when the worker cannot render.** `rendererAvailability()` at prepare time records `RENDERER_UNAVAILABLE` on the artifact instead of enqueueing a job that would fail three times.

## 2026-09-11 — Report copy fixed at the template layer; release waits for the video; Laura Kirsch held (spec 139)

- **Semantic concerns are fixed in the template, not accepted per prospect.** The reviewer v2 blocks on Kirsch Team's report were generic spec 128 wording ("AI recommends…", "Production vs AI recommendations", "audit", team/agent switching in the change section). They are corrected once in `changeFirstRows`, the report component and the serialized review text; the reviewer is not weakened. Founder acceptance remains, now bound to the exact content hash it accepted — a revised artifact is reviewed again; evidence/assertion blocks are never acceptable.
- **Entity wording is deterministic** (`entityRef`: team / individual / brokerage; unknown reads as team). The measured entity is the production-record business; the recipient's name is only the greeting.
- **Release policy defaults to report_and_video.** A handoff holds at `qa_passed` (`WAITING_FOR_VIDEO`) until the video lane's own release recheck says the walkthrough is releasable; then the email is re-staged as the video variant (new intent, report-only draft superseded) and re-reviewed, and the lane mode decides. A releasable video never transmits by itself. `report_only` must be set explicitly.
- **Laura Kirsch is held, not sent.** Draft 09d355fb unscheduled through the existing cancel action (approved, audit trail intact); evidence unchanged (team, 2/25/256); report republished on the corrected template as a new revision. Final delivery requires REPORT + VIDEO gates and the founder's release.
...
## 2026-09-13 — Touch 1 supply engine: full RealTrends universe, market policy, contact-before-benchmark
- **The top of the funnel is `realtrends_records`, not `prospects`.** 74,847 licensed records (20,621 teams / 54,226 individuals, production year 2025, 5,192 city+state geographies); only 330 were prospects. `lib/prospects/supply-engine.ts` normalizes source records deterministically (exact identity = name + type + city + state; same-production duplicates collapse, conflicting-production duplicates fail closed as AMBIGUOUS; a person who is a team's RealTrends-stated lead in the same geography is the same buying unit). No fuzzy merge, no company created for the universe — an entity earns a prospect row only through `scripts/supply-promote.ts` after its contact is verified.
- **Market control is derived, not invented (`lib/prospects/market-policy.ts`).** TIER_1_RESERVED comes from the explicit registry (only NYC + its boroughs, founder directive 2026-09-11, state-qualified so "Manhattan, KS" is not New York); ACTIVE_CLIENT_EXCLUSIVE from live spec-052 scopes; PAUSED from launch status; PROOF_BUILDING from an unarchived outbound-capable launch (launch kickoff is the confirm-gated founder act that opens a market); everything else is UNCLASSIFIED — research allowed, outbound blocked. OPEN_FOR_SCALE exists in the model and is never derived because no founder decision assigns it. `PROTECTED_MARKET_NAMES` in t1-cohort now re-exports this registry. Why: a DB-backed registry would need a migration numbered above the two in-flight branches (113/114) and a founder classification pass; nothing here needed either.
- **Contact verification precedes benchmark spend.** Contactability (CONTACT_VERIFIED / CONTACT_RESEARCH_REQUIRED / NO_CONTACT_FOUND / GENERIC_ONLY / CONTACT_UNVERIFIED) reuses spec-042 labels: only `verified`/`publicly_sourced` on a non-generic address is contactable. A fresh run or refresh is JIT-authorized for a market only when it holds ≥ the derived minimum of contact-verified prequalified entities.
- **Wave sizes are derived, never chosen.** Yield per contact-verified entity = (47/203)·(32/47) from t1-cohort-002 = 0.158; minimum wave = ⌈1/yield⌉ = 7 (one benchmark expected to yield ≥ 1 QUALIFIED_T1), target = ⌈BROKERAGE_SEND_CAP_30D/yield⌉ = 20. Re-derive from each frozen cohort.
- **The $20M production floor ranks, it never excludes.** It orders wave depth and the contact-research queue; t1-cohort-002 qualified a $15.4M team, so a floor as a gate would have thrown away real supply.
- **Qualification and dispatch are different states.** `dispatchPlan` turns a full per-market brokerage bucket into DEFERRED_QUALIFIED with `nextEligibleSendAt` (the oldest counted send + 30 days) and `refreshRequiredBeforeSend` when that date passes the benchmark's 14-day expiry. Confirmed: the spec-120 cap is enforced at selection (`selectCohort`) and at send time only — it never touches mismatch evaluation or evidence release, so it moves dates, not grades.
- **Nothing in the engine starts a run or touches outreach.** `scripts/supply-engine.ts` is read-only; promotion attaches a verified entity to the market benchmark project as a competitor entity (frozen answers are re-resolved by backfill) and refuses while a run on that project is pending/running. A concurrent session was already creating the 2026-09-14 refresh runs, so no run was started here.

## 2026-09-13 — Public site numbers come only from an approved-claims registry

Every figure on recommendedfirst.com (research pages, FAQ, methodology) is read from `lib/marketing/approved-claims.json` through `lib/marketing/claims.ts`; a claim that is not `approved` throws at render, and `scripts/marketing-claims-verify.ts --check` recounts each claim from the immutable measurement tables with the canonical eligibility rules (valid cells, non-holdout prompts, current revision, echo exclusion). Why: the site is meant to be an evidence document; hardcoded numbers drift silently, and the platform already had the canonical counting functions. Corollaries: no prospect, team or client is ever named publicly; API captures are labeled "the OpenAI model", never "ChatGPT"; `Dataset` structured data is emitted only on pages with a described method and a denominator-bearing table; `ProfessionalService`/`LocalBusiness` schema is withheld until a postal address is verified; `llms-full.txt` is not generated (would duplicate page bodies and go stale). Crawler files (`robots.txt`, `sitemap.xml`, `llms.txt`) are public in middleware and derive from the single page registry in `lib/marketing/constants.ts`.

## 2026-09-14 — Founder opens additional markets for the Touch 1 supply engine
- **Founder instruction (2026-09-14, chat):** "proceed to other markets so we can get 100 T1 prospects … go to as many other markets as needed." This is the founder act that moves a market from UNCLASSIFIED to PROOF_BUILDING; the engine records it as a launch (the existing confirm-gated kickoff), never as a code-level classification. NYC stays TIER_1_RESERVED regardless.
- **Sequencing for NEW markets: benchmark first, then contacts — a deliberate exception to contact-before-benchmark.** In a market with no evidence, blind contact research costs roughly ten times a market benchmark and yields ~0.16 qualified per verified contact; a $5 market run followed by the zero-presence scan (fresh-prospect-discovery pool B) names the entities that would qualify, and contact research then runs only on those (~0.4 verified per entity). Freshness risk is bounded: the dispatch layer flags refresh-required and a refresh costs the same $5. In markets that already hold a fresh benchmark the order stays contacts-on-hook-ready-only.

## 2026-09-14 — Contact discovery cascade, provider preflight, identity invariant
- **Fetch failure is not contact failure.** `lib/prospects/contact-discovery.ts` classifies a 403/429/503 as HOST_BLOCKED and a JS shell as JS_RENDER_REQUIRED; both route to the next tier (search-discovered official page, headless render via the video lane's Playwright Chromium). NO_CONTACT_FOUND is only reachable after every applicable tier was tried; a blocked host with untried tiers is DISCOVERED_UNVERIFIED.
- **Discover broadly, verify strictly.** Discovery uses code only: deterministic queries over Brave's public results (DuckDuckGo lite returns a challenge, Bing is a JS shell), aggregator/social/enrichment-database hosts dropped, press-release wires and agent-profile aggregators demoted to review pointers. Extraction is mailto / JSON-LD / literal text / Cloudflare-encoded, by regex. The ONE verification rule stays `contact-verify` (literal on the fetched page, or on the rendered DOM with `sourceType: rendered_page` recorded). No model is called anywhere in the cascade; LLM use is reserved for identity ambiguity and was 0 in the first 86 candidates.
- **Binding is by distinctive tokens only.** The first smoke batch bound another team's Gmail to a team because "group" matched; generic name words never bind, matches must hit the local part, the domain, or the person's printed full name (and never a free-mail domain on name alone). Pinned by tests.
- **Research is cached per candidate** (`.local-data/supply/contact-research-cache.json`) with per-route retry windows (transient 1 day, blocked 14, no-email 30) so a failed route is not hammered; `--revalidate` re-applies current binding rules to cached finds.
- **Provider preflight is a 1-token probe, cached 30 minutes** (`scripts/provider-preflight.ts`): OpenAI and Perplexity both CAPACITY_BLOCKED on 2026-09-14; benchmarks stay parked, contact research continues. Billing is never touched by code.
- **Identity invariant is now a pure function** (`promotionIdentityCheck`): a machine match must carry the record's exact name, a person-named company never absorbs a TEAM record, and a company measured at another level never absorbs a record of a different level. A lead relationship is recorded on the team and never changes the measured entity.
- **Market expansion is a founder queue, not a switch.** `marketExpansionClass` labels UNCLASSIFIED markets PROOF_BUILDING_CANDIDATE / PREMIUM_RESERVE_CANDIDATE (≥ 15 teams over $100M — the line between the six largest metros and the rest) / RESEARCH_MORE / LOW_PRIORITY with reasons; the six staged metros were NOT opened.

## 2026-09-14 — Citation readiness (spec 141): downgraded re-parses are flagged, not hidden; vendor pages quote, never judge

The claims verifier now reports `INTEGRITY` when a run's current mention revision comes from a weaker parser than a superseded revision (heuristic v1 over LLM v2), and `--strict` fails on it. The current-revision rule stays canonical, so the registry adopts the recounted values with a dated `updateHistory` entry and the public pages print that history; the fix belongs upstream (re-run the LLM parser as a new revision), never in the registry. Why: on 2026-09-13/14 a heuristic re-parse rewrote several historical runs and silently moved public numbers; a registry that hid that would be exactly the drift it exists to catch.

Comparison pages (`/recommended-first-vs-*`, `/best-ai-visibility-tools-for-real-estate`, `/recommended-first-alternatives`) contain only vendor self-descriptions with source URL and retrieval date (`lib/marketing/vendors.ts`) and Recommended First's verified attributes; no ranking, no evaluation, no untested feature claims. The flagship route keeps the public name "ChatGPT Real Estate Visibility Benchmark" and states the instrument (OpenAI web-search API) in its first sentence and in the verbatim `INSTRUMENT_NOTE` on every benchmark page.

Public dataset v1.0.0 is aggregate only: entities anonymized per benchmark, agent-owned and single-market domains bucketed, no RealTrends fields, no answer or prompt text; answer-level rows are withheld pending a privacy review. Downloads are generated from the committed JSON so the site and the repository copy cannot diverge. Migration 115 (citation map, intervention controls) is written but not applied; no external submission, outreach or directory listing is performed from the repository.
- **Search discovery capacity is the real ceiling, not email availability.** Brave's public results (the one engine that returns links to a plain fetch) rate-limited this IP after ~250 queries and now serves a CAPTCHA; DuckDuckGo html/lite return an anomaly challenge, Bing is a JS shell (and refuses headless), Startpage returns no result links, Mojeek 403, Yahoo 500. The runner detects a rate limit, backs the engine off 20 minutes, stops the batch cleanly, and leaves untouched candidates PENDING. A supplied-URL mode (`--urls-file`) lets an approved search step hand over page pointers only; extraction, binding and verification never leave code. Rendered-page verification (`rendered_page`) turned five 403-to-curl "blocked" hosts into verified own-domain contacts today.
- **Backfill audit (2026-09-14 promotion job).** The script's own work took 26 s (12:09:03→12:09:29: 5 contacts, 1 prospect, 1 attach); the process then sat idle 14 min because the headless browser was never closed (fixed). The slow step is not the script: `addCompetitor` → `backfillProject` deletes every `response_parses` row of up to 12 recent runs on the project and enqueues a `parse_response` job per response (1,238 jobs for one attach), each of which calls the LLM classifier; the single serial worker clears ~11/min, so one attach costs ~2 hours of parse queue. Two defects surfaced: (1) the Wilmington NC and DE launches share one benchmark project (`bootstrapMarketBenchmark` names the project by bare market name, so same-named cities in two states collide — the 08-21 Wilmington lesson, one layer down), so the NC attach re-parsed 726 Delaware responses; (2) with the classifier provider blocked, the reparse replaced LLM parses with heuristic ones (99 of the first 169). Safe changes made: `addCompetitor` gained `backfill` (default true, unchanged for every existing caller); the promotion script attaches without backfill, runs ONE backfill per touched project at the end of a batch, and defers it entirely while the cached preflight says the classifier provider is blocked. Not changed: the parser itself (parse-once/index-once would mean an incremental per-company mention pass keyed on run + parser version + alias state — worthwhile, but not while a live queue is draining) and the shared-project keying (P1: key the benchmark project by market id, and split the Wilmington project).

## 2026-09-14 — Public numbers use verified-only classification precedence; revision 3 is offline text corroboration

Public claims and the public dataset read the highest VERIFIED revision of each (response, company) pair (human review > explicit `verification_status='verified'` > classifier rows ≥ 0.7 with no review flag); heuristic rows never count; a pair whose newest row needs manual review blocks every claim over its run. Internal operator views keep the current-revision rule. Why: a heuristic re-parse with no LLM key wrote revisions 2–5 over classifier rows on nine runs and silently moved public numbers; row-level comparison against the answer text showed the heuristic produced false negatives. Revision 3 (`mention-parser-v3+adjudication`) was produced offline because the OpenAI account had no credits: the classifier judgment is retained only where the text corroborates it (alias present, quoted excerpt grounded, confidence ≥ 0.9); everything else goes to manual review, and pairs with no classifier judgment are `not_classified` (excluded, never zero). New columns (migration 116) are additive; no row was edited. Staging (a mirrored local database) is where the recount, dataset v1.0.1 and the release gate were exercised; production is untouched until a person applies the migration and artifact.

## 2026-09-14 — Markets are identities, parse once / resolve many, provider failure never downgrades evidence

**Incident.** A promotion batch attached companies to the "Market benchmark: Wilmington" project. That project was keyed on the bare display name, so Wilmington, NC and Wilmington, DE shared it, and an NC attach re-parsed DE answers. `addCompetitor` ran a delete-and-reparse backfill (drop every `response_parses` row of ≤12 runs, re-enqueue one `parse_response` per answer: 1,238 jobs for one attach at ~11/min). The classifier provider was blocked during part of the drain, and the parse fallback appended heuristic revisions on top of LLM judgments (1,612 pairs / 754 responses on Wilmington, 1,658 / 472 on the Properties by Southern Jersey City project; all on 2026-09-14). No draft, send, or published audit consumed a downgraded count (published audits are frozen snapshots from ≤ 2026-09-06; no drafts after 2026-09-13 for either project). The 617 still-queued Wilmington parse jobs were paused (`run_after` +30 days) as the first fail-closed step.

**Decisions.**
1. **Project identity is `projects.market_id` (markets.id), never the label** (migration 117). `bootstrapMarketBenchmark` looks up and binds by market id; a legacy name-keyed project that is not bound and shares its name across markets is `PROJECT_MARKET_REVIEW_REQUIRED` — it is never adopted and never silently split. Migration 117 binds only projects whose name maps to exactly one market and one project; the Wilmington project stays unbound (runs 66af5cdb/DE with 20 DE + 2 NC prospects, 62db6814/NC with 24 NC prospects) and needs a founder decision to split. `scripts/supply-promote.ts` resolves the launch's project by market id and refuses unbound legacy projects.
2. **The current revision of a pair is class-first.** `CURRENT_REVISION` (db/mentions.ts), `authoritativeRevision` (lib/parsing/precedence.ts) and the shadow recount each state, independently, that a classifier-class row (LLM v2, adjudication v3, or human-reviewed) is never superseded by a heuristic row, whatever its revision number; within a class the newer revision wins. History is untouched — the heuristic rows stay, legibly stamped, and are simply not selected. This restores every downgraded count at read time with no data mutation. Public claims keep spec 141's stricter verified-only precedence.
3. **No downgrade at write time either.** With a classifier key configured, a classifier failure on a response that already carries a classifier-class judgment throws `ClassifierUnavailableError` (job retries with backoff; nothing written). A first parse of a response with no classifier history keeps docs/12 graceful degradation (heuristic, truthfully stamped, upgradeable). A missing ledger row whose judgments still exist is **reconstructed from the immutable revisions** (`reconstructed_from_mentions = true`), never re-classified.
4. **Attach = resolution, not re-parse.** `addCompetitor` enqueues ONE `backfill_company` job per (project, company), deduplicated against the queue. The job runs `parseCompanyIntoRun` (alias prepass → classifier only where the company is named; judged pairs reused; heuristic-only pairs upgraded) under a per-(run, company) advisory lock, and records a `run_company_parses` row keyed by the company's alias-graph hash. Invalidation is dependency-driven: alias change → re-resolve that company (new hits only, via `enqueueAliasBackfills` from `applyVerifiedAliases`); parser-version change → `enqueueParseJobs` (semantic layer); another company attached → nothing. Every execution is a `company_backfills` row (scope, reuse, classifier calls, provider state, duration, result); > 400 classifier calls in one backfill is `review_required`, not a fan-out.
5. **Browser lifecycle is structural.** `renderPageHtml` closes page and context in `finally` on every path; the shared Chromium closes itself after 15 s idle (unref'd timer); `withRenderer(fn)` scopes it to a block. Scripts wrap `main` in `withRenderer`. `scripts/render-smoke.ts` is the process-exit regression against real Chromium.

**Architectural debt (documented, not built).** Semantic classification is still per (response × candidate companies): the classifier judges a named candidate set, so "parse once, resolve many" is realised at the prepass/ledger layer rather than by an entity-agnostic mention extraction. A future parser that emits all named businesses per answer, resolved afterwards against the alias graph, would make alias-only changes free of classifier calls even for newly matching answers. Not needed to stop the incident; recorded so the next step is deliberate.

## 2026-09-14 — Deployment identity is asserted at runtime; Wilmington legacy project is bound to DE by immutable prompt provenance

**Runtime identity.** `worker_heartbeats.version` / `handlers` (migration 118) are written by the running worker from `BUILD_COMMIT` (set on the Railway service before `railway up`) and from its handler registry. RUNNING_COMMIT_MATCHES_EXPECTED and REQUIRED_JOB_HANDLERS_PRESENT are read from that row, never inferred from the working tree.

**Wilmington.** Every run of the legacy "Market benchmark: Wilmington" project (2557818a, 66af5cdb, 62db6814, 843c7496) was executed over frozen prompts that name Delaware (4/9/9/9 prompts) and never North Carolina, from the only installed pack (hierarchy Delaware). By the provenance rule — a run's geography is what its immutable prompts ask about, not its label — the project is bound to Wilmington, DE (`scripts/wilmington-reconcile.ts --apply`, audited as `project.market_bound`). The run labelled "Cohort 124 batch 1: Wilmington, NC" (62db6814) is therefore a DE run; its 24 NC prospect bindings and the 2 NC bindings on 66af5cdb (26 prospects, 0 sends, 0 published audits) are cross-market contamination from the display-name collision. They stay in history, are blocked structurally by the new release check `BENCHMARK_MARKET_VERIFIED` (a market-level project's run may only back prospects of its own market), and cannot recur: `linkBenchmark` refuses cross-market bindings and `bootstrapMarketBenchmark` refuses a pack whose hierarchy names another state (`PACK_MARKET_MISMATCH`). `PROJECT_MARKET_ISOLATION` distinguishes active collisions (BLOCK) from these quarantined historical bindings (reported). Wilmington, NC has no canonical project until an NC pack is installed and benchmarked — a founder action, not a script.

**Queue.** The 617 paused parse jobs are classified by `scripts/paused-jobs-release.ts` before any resume; only NO_OP and ENTITY_RESOLUTION_ONLY categories may be released without a fresh provider preflight, in bounded batches.
