/**
 * Legacy front door (spec 076/032, exchanged by spec 134): /audit/<slug>/<key>/walkthrough.
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ handle: string; key?: string }> }): Promise<Response> {
  ...
  return handleInvitation(request, { credential: p.key ?? "", slugHint: p.handle, next: "walkthrough" });
}
