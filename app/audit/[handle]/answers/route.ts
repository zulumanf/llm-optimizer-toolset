/**
 * Legacy front door (spec 076/032, exchanged by spec 134): /audit/<token>/answers.
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ handle: string; key?: string }> }): Promise<Response> {
  ...
  return handleInvitation(request, { credential: p.handle, slugHint: null, next: "answers" });
}
