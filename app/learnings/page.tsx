...
export default async function LearningsPage() {
  ...
  return (
    <PageShell>
      <PageHeader
        title="Learnings"
        description="Measured outcomes, distilled. Confirmed learnings about a play move its rank in every future plan — supports up, cautions down — so the second engagement starts smarter than the first."
        actions={<RecordLearningDialog />}
      />

      <Section title="Active">
        {active.length === 0 ? (
          <EmptyState message="Nothing recorded yet. When a re-measured intervention settles, distill what it showed here — cite its outcome IDs and future plans will use it." />
        ) : (
