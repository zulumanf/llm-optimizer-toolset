...
## Knowledge Graph
Entity graph across responses: companies ↔ sources ↔ claims ↔ topics, revealing *why* providers associate brands with categories ("the client co-occurs with X in citations from domain Y"). Postponed because: raw tables answer today's questions; a graph without a question is decoration. Revisit when we repeatedly ask relationship questions SQL makes painful.
...
## PR Automation
Findings → automatically drafted content changes / PRs to the client's site or docs. Postponed: highest-risk loop closure; requires mature attribution (spec 007) proving which changes matter before automating them.
...
## Customer SaaS
Selling this as a product. Explicit non-goal (`docs/00-vision.md`): multi-tenancy, billing, and support would consume the roadmap. The internal moat (methodology + longitudinal data) matters more. Reconsider only after the agency itself demonstrably benefits for 6+ months.
