...
|---|---|---|---|---|---|---|
| 2.1 Prospect-owned benchmark runs — **done 2026-08-02** | Run fresh benchmarks for prospects with no existing coverage | Shipped as migration 041: `projects.kind='prospect'`; `listCompaniesForProject` excludes only *client* subjects; `createBenchmarkProject` composes project+subject+launch-rival competitors (no `startRun` change needed — kind is orthogonal to status); prompt set stays human-built in the project workspace | Slice | High (touches measurement core; needs regression tests on client scores) | L | Verified: client scores byte-identical before/after a prospect project exists (`tests/integration/prospect-benchmark-projects.test.ts`) |
| 2.2 CSV import + research profile — **done 2026-08-02** | Faster market coverage | Shipped as migration 042 + `lib/prospects/import.ts` (pure RFC-4180 parser, 200-row cap) + `importProspects` (persists through the same `createProspect`/`addContact` paths as manual entry; per-fact provenance labels; per-row created/duplicate/error report; import audited with counts + source URL) | 1.1 | Low | M | Verified: `tests/unit/prospect-import.test.ts`, `tests/integration/prospect-contacts.test.ts` |
| 2.3 Prospect contacts — **done 2026-08-02** | Real outreach targets | Shipped: `prospect_contacts` (role/channel prefs, per-contact DNC, provenance, one-primary + unique-email partial indexes); drafts take an optional `contact_id` (same-prospect rule enforced in service — the DB cannot express it); `approveOutreachDraft`/`recordDraftSent` now gate on account DNC → contact DNC → global `suppression_entries` match on normalised contact identifiers | 1.1 | Low | S | Verified: suppression on `x+tag@y.com` blocks a draft to `x@y.com` (`tests/integration/prospect-contacts.test.ts`) |
