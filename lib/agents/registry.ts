...
export const AGENTS: AgentDefinition[] = [
  ...
  {
    key: "client_communication_reviewer",
    name: "Client Communication Reviewer",
    mission:
      "Read one client-facing update against its canonical fact pack; report unsupported claims, causal overclaims, contradictions, jargon and sales language. Advisory: the founder sends.",
    domain: "advisory",
    version: "client-communication-review-v1",
    model: AGENT_MODEL,
    status: "implemented",
    module: "lib/engagements/reviewers.ts",
    inputSchema: z.object({ draft: z.string(), factPack: z.string() }),
    outputSchema: z.object({ pass: z.boolean(), issues: z.array(z.object({ kind: z.string(), quote: z.string(), why: z.string() })) }),
    allowedTools: [],
    allowedDataScopes: ["client_fact_pack", "client_draft"],
    prohibitedActions: [...NEVER_WRITE, "retrieve facts outside the pack", "rewrite the draft", "send anything"],
    evidenceRequirements: ["the fact pack assembled by the portfolio service"],
    escalationConditions: ["any P0 deterministic issue exists (the reviewer never overrides it)"],
    maxCostMicroUsd: 200_000,
    maxSeconds: 60,
  },
  {
    key: "client_evidence_reviewer",
    name: "Client Evidence Reviewer",
    mission:
      "Read client-facing interpretation of measurement evidence against the canonical numbers and comparability verdict; flag causality the evidence does not establish. Advisory only.",
    domain: "advisory",
    version: "client-evidence-review-v1",
    model: AGENT_MODEL,
    status: "implemented",
    module: "lib/engagements/reviewers.ts",
    inputSchema: z.object({ text: z.string(), factPack: z.string() }),
    outputSchema: z.object({ pass: z.boolean(), issues: z.array(z.object({ kind: z.string(), quote: z.string(), why: z.string() })) }),
    allowedTools: [],
    allowedDataScopes: ["client_fact_pack", "client_text"],
    prohibitedActions: [...NEVER_WRITE, "retrieve facts outside the pack", "decide comparability", "change counts"],
    evidenceRequirements: ["frozen baseline and measurement packages via the fact pack"],
    escalationConditions: ["comparability is not HIGH and the text claims before/after"],
    maxCostMicroUsd: 200_000,
    maxSeconds: 60,
  },
];
