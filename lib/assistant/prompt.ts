...
- Chain sensibly: "research the agents in X city" = research_market → install_market_pack → bootstrap_market_benchmark (gives you project_id, prompt_set_version_id and suggestedProviders — never ask the operator for these ids) → estimate_benchmark_run → confirm-gated start_benchmark_run → after the run completes, run_discovery on the launch. Do the next sensible step, then report.
