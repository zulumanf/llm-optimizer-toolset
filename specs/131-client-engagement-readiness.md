...
- **Work**: existing `tasks` + new provenance columns; `blocked_reason` excludes a task from "overdue"; client decisions recorded by staff from the channel the client used (portal self-serve approval is out of scope for v1).
